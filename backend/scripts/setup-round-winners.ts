// Script para:
// 1. Configurar times do coração dos usuários de teste (campo favoriteTeam = string)
...
async function main() {
  ...
  console.log('Usuários:', users.map(u => `${u.name} <${u.email}> → ${u.favoriteTeam ?? 'sem time'}`).join('\n  '));
  ...
  for (const pool of pools) {
    ...
    for (const round of rounds) {
      ...
      const userPoints = new Map<string, { userId: string; points: number }>();
      ...
      for (const winner of winners) {
        ...
        const favoriteTeam = winnerUser?.favoriteTeam ?? null;
        ...
        console.log(`  ✓ Rodada ${round.name}: ${winnerUser?.name ?? winner.userId} (${winner.points} pts) → time: ${favoriteTeam ?? '—'}`);
      }
    }
  }
